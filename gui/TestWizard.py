...
def load_device_objects(path=None, cfg=None):
    """Return configured device instances.

    Parameters
    ----------
    path : str, optional
        Path to a Python file defining device instances. If supplied, the
        instances are imported from this file.
    cfg : dict, optional
        Configuration dictionary used to build device objects when ``path`` is
        not provided. Typically this is ``self.cfg`` from ``TestWizard``.
    """
    ...
    if path:
        if not os.path.exists(path):
            return {}
        try:
            spec = importlib.util.spec_from_file_location("Test_Cell_Devices", path)
...
class TestWizard(tk.Tk):
    def __init__(self, test_name=None, test_dir=None, load_path=None, device_file=None):
        super().__init__()
        ...
        self.device_file = device_file or os.path.join(REPO_ROOT, "config", "Test_Cell_1_Devices.py")
        # Build objects either from provided script or current configuration
        self.device_objects = load_device_objects(self.device_file if os.path.exists(self.device_file) else None, self.cfg)
        self.setup_code = ""
