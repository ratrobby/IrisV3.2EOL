...
class DeviceTab(ttk.Frame):
    def __init__(self, master, cell_name):
        ...
        name_frame.place(relx=0.02, rely=0.50, relwidth=0.46)
        ttk.Label(name_frame, text="Test Name:").pack(side="left", padx=(0, 5))
        self.test_name_var = tk.StringVar()
        ttk.Entry(name_frame, textvariable=self.test_name_var).pack(side="left", fill="x", expand=True)
        ...
        ttk.Button(self, text="Run Test Loop", command=self.run_test_loop).place(relx=0.15, rely=0.86)
    ...
    def _load_devices(self):
        devices = []
        path = DeviceUtils.OUTPUT_FILES[self.cell_name]
    ...
    def _discover_methods(self, devices):
        setup = []
        commands = []
        for dev in devices:
    ...
    def run_test_loop(self):
        ...
        setup_methods, command_methods = self._discover_methods(devices)
        ...
        with open(log_path, "w") as f:
            ...
            f.write("Device Setup:\n")
            for dev, meth in setup_methods:
            ...
            for i in range(3):
                ...
                for dev, meth in command_methods:
                    try:
                        result = meth()
                        f.write(f"  {dev.__class__.__name__}.{meth.__name__}: {result}\n")
                    except TypeError:
                        f.write(f"  {dev.__class__.__name__}.{meth.__name__}: missing parameters\n")
                    except Exception as e:
                        f.write(f"  {dev.__class__.__name__}.{meth.__name__}: ERROR {e}\n")
